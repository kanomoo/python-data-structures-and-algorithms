...
## 📝 5. คลังตัวอย่างข้อสอบ & แบบฝึกหัดในห้องเรียน (Classroom "For Example 5.1" Tracing)

> [!INFO]
> **ที่มาของเอกสารต้นฉบับ**: เอกสารใบงานในห้องเรียนจริง `Lectures/Lecture 5.1 Binary Search Tree/For example Binary Search Tree std delete round 4 in insert_s step.pdf`

---

### 🎯 5.1 โจทย์ For Example 5.1: การ Trace พารามิเตอร์ของ `_insert_recursive()`

#### 📄 โจทย์คำถาม (Problem Statement):
> If there is an existing `BinarySearchTree`'s instance variable named `bst` containing the numbers `[1, 2, 3, 4, 6, 8]` (In-order traversal) as shown in the tree structure below:
> 
> Write the steps for the `_insert_recursive()` function when inserting `5` into the instance variable `bst`. 
> In each round, write down 2 things:
> 1. What are the values of the parameters `value` and `current_node` of the `_insert_recursive()` function?
...
graph TD
    ...
    style N4 stroke:#f59e0b,stroke-width:2px
```

---

#### 💻 เฉลยคำตอบทีละรอบตามเกณฑ์ตรวจของอาจารย์ (Step-by-Step Iteration Answer)

| รอบที่ (Round) | ค่าพารามิเตอร์ (`value`, `current_node`) | เงื่อนไข if-else ที่ประเมินเป็นจริง (True) | ประโยคคำสั่งที่ถูกเรียกทำงาน (Executed Statement) |
| :---: | :--- | :--- | :--- |
| **Round 1** | `value = 5`, `current_node = 6` | `value < current_node.val` ($5 < 6$ จริง) | `_insert_recursive(current_node.left, value)` |
| **Round 2** | `value = 5`, `current_node = 2` | `value > current_node.val` ($5 > 2$ จริง) | `_insert_recursive(current_node.right, value)` |
| **Round 3** | `value = 5`, `current_node = 4` | `value > current_node.val` ($5 > 4$ จริง)<br>และ `if current_node.right is None:` เป็น **True** | `current_node.right = Node(value)` |
...
graph TD
    ...
    N2 --> N4["4"]
    N4 --> N3["3"]
    N4 -->|แทรกใหม่รอบที่ 3| N5["5 (โหนดใหม่)"]
    
    style N5 fill:#10b981,stroke:#059669,stroke-width:3px,color:#fff
```

> [!WARNING]
> **กับดักอาจารย์ในสไลด์หน้า 3 (ขีดฆ่าเตือนใจ)**:
> ในชีทของอาจารย์มีรอยขีดฆ่าสีแดง: `round 4: value = 5, current_node = None` 
> **นี่คือจุดที่นักศึกษาเขียนผิดกันเยอะมาก!**
> ในโค้ดแบบ Recursive ที่ตรวจสอบ `if current_node.right is None:` ล่วงหน้า การสร้างโหนดจะเกิดขึ้นที่รอบ 3 ทันที ไม่มีการเรียก recursive รอบที่ 4 ส่ง `None` เข้ามา!

---

#### 🖥️ ตัวอย่างหน้าจอ Interface การจำลองการแทรกค่าใน BST (Interactive Preview)
```console
============================================================
              BST RECURSIVE INSERTION TRACER               
============================================================
[Target Value to Insert] 5
[Trace Round 1] current_node: 6 (Root)
                Evaluation: 5 < 6 -> Branch LEFT to node 2
[Trace Round 2] current_node: 2
                Evaluation: 5 > 2 -> Branch RIGHT to node 4
[Trace Round 3] current_node: 4
                Evaluation: 5 > 4 (Branch RIGHT)
                Check: node.right is None -> TRUE
                Action: Created Node(5) as RIGHT child of 4!
------------------------------------------------------------
[Final In-order] [ 1, 2, 3, 4, 5, 6, 8 ] (Sorted Property Verified)
============================================================
```
...
- ก่อนหน้า: [[05.1 - Tree Terminology & Binary Trees]]
- ถัดไป (BST Deletion): [[05.3 - Binary Search Tree Removal (Node Deletion Cases)]]
